...
const CollaboratorPanel = ({ isOpen, onClose, users }) => {
  ...
  return (
    <div
      className={`fixed top-22 sm:top-2 flex flex-col gap-2 w-64 sm:w-72 h-[85vh] sm:h-[97vh] bg-gray-900/80 backdrop-blur-md border border-gray-800 rounded-xl shadow-xl transition-all duration-300 z-40 overflow-hidden ${isOpen ? "left-2" : "-left-72 sm:-left-80"
        }`}
      ref={panelRef}
    >
      {/* Header */}
      <header className="flex justify-between items-center px-3 py-2 bg-gray-800/80 rounded-t-2xl flex-shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-2 hover:text-red-400 transition cursor-pointer"
        >
